...
from src.utils import celeb_names_cross_init
...
def import_model_class_from_model_name_or_path(pretrained_model_name_or_path: str, revision: str):
    ...
    if model_class == "CLIPTextModel":
        #CR_: import their clip model
        # from transformers import CLIPTextModel
        from src.clip_model import CLIPTextModel
...
def parse_args(input_args=None):
    ...
    #adding cross initialisation arguments
    parser.add_argument(
        "--celeb_path", 
        type=str, 
        default=None, 
    ...
    parser.add_argument(
        "--cross_initialised",
        action="store_true",
        help="Cross initialised text embedding",
    )
    ...
    if args.with_prior_preservation:
        ...
        if args.class_data_dir is not None:
            print("You need not use --class_data_dir without --with_prior_preservation.")
        if args.class_prompt is not None:
            print("You need not use --class_prompt without --with_prior_preservation.")
...
def main(args):
    ...
    #adding cross_initialisation tokens
    if args.modifier_token is not None:
        instance_modifier_tokens = args.modifier_token.split('+')
        print(f"Instance_modifier_tokens: {instance_modifier_tokens}")
        for instance_modifier_token in instance_modifier_tokens:
            new_instance_modifier_tokens = []
            for id in range(args.n_persudo_tokens):
                new_instance_modifier_tokens.append(instance_modifier_token + f'_v{id}')
            new_instance_modifier_tokens = " ".join(new_instance_modifier_tokens)
            #find the instance_modifier_token iun each concept list
            for concept in args.concepts_list:
                #To be more robust, we will need to make sure each modifier token is unique to each concept
                if instance_modifier_token in concept["instance_prompt"]:
                    print("Before:", concept["instance_prompt"])
                    concept["instance_prompt"] = concept["instance_prompt"].replace(instance_modifier_token, new_instance_modifier_tokens)
                    print("After:", concept["instance_prompt"])
        ...
        for i, concept in enumerate(args.concepts_list):
            ...
            if args.real_prior:
                ...
                if cur_class_images < args.num_class_images:
                    ...
                    generated_class_prompt = f"photo of a {concept['class_prompt']}"  
                    sample_dataset = PromptDataset(generated_class_prompt, num_new_images)
    ...
    if args.modifier_token is not None:
        ...
        print(f"Modifier tokens: {args.modifier_token}")
        for modifier_token, initializer_token in zip(args.modifier_token, args.initializer_token[:len(args.modifier_token)]):
            # Add the placeholder token in tokenizer
            #with cross initlization, intialise it with the pseudo tokens
            for id in range(args.n_persudo_tokens):
                new_modifier_token = modifier_token + f'_v{id}'

                num_added_tokens = tokenizer.add_tokens(new_modifier_token)
                if num_added_tokens == 0:
                    raise ValueError(
                        f"The tokenizer already contains the token {new_modifier_token}. Please pass a different"
                ...
                cross_initialized_mod_tokens.append(new_modifier_token)
                modifier_token_id.append(tokenizer.convert_tokens_to_ids(new_modifier_token))
        args.modifier_token = cross_initialized_mod_tokens

        print(f"Modifier tokens (after CR): {args.modifier_token}")
        # Resize the token embeddings as we are adding new special tokens to the tokenizer
        text_encoder.resize_token_embeddings(len(tokenizer))

        #initialise token embedding with celeb embedding
        ...
        #     #CR_: initialise celeb embeddings from line 213
        #     token_embeds[x] = token_embeds[y]

        #instead of initialixing the weight with the initializing token, 
        token_embeds[modifier_token_id]=initialize_embeds
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, text_encoder):
                ...
                reg_loss=F.pairwise_distance(token_embeds[modifier_token_id],initialize_embeds,p=2).mean()*args.reg_weight
                ...
                if args.with_prior_preservation:
                    ...
                    #compute text encoding loss here, note the reg_weight
                    
                    #text_emebdding_reg_loss=F.pairwise_distance(token_embeds[placeholder_token_ids],initialize_embeds,p=2).mean()*args.reg_weight

                    # Compute prior loss
                    prior_loss = F.mse_loss(model_pred_prior.float(), target_prior.float(), reduction="mean")

                    # Add the prior loss to the instance loss.
                    loss = loss + args.prior_loss_weight * prior_loss +reg_loss
                    # loss = loss + args.prior_loss_weight * prior_loss + text_emebdding_reg_loss
                else:
                    mask = batch["mask"]
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="none")
                    loss = ((loss*mask).sum([1, 2, 3])/mask.sum([1, 2, 3])).mean()
